...
<xsl:template match="instantiationIdentifier[@source='File Name']">
	<pbcoreCollection>
	    ...
	    <xsl:otherwise>
	        <xsl:analyze-string select="." regex="bmac_([\w_-]+)_">
...
<!-- Stores item ID as variable and then adds to ARCHive URI prefix-->
    <!-- regex to get the filename without the extension for CA matching -->
        <xsl:variable name="item-id">
            <xsl:analyze-string select="." regex="bmac_(.+)\.\w{{3}}">
            <xsl:matching-substring><xsl:value-of select="regex-group(1)" /></xsl:matching-substring>
             </xsl:analyze-string>
         </xsl:variable>
...
<!-- Sets Generation type -->
 ...
 <!-- Sets Creator to Crawford -->
    <instantiationCreator>
        <xsl:text>crawford</xsl:text>
    </instantiationCreator>   
...
-->
 ...
 <xsl:template match="instantiationDigital">
        <xsl:choose>
            <xsl:when test=". = 'application/mxf'">
                <instantiationDigital>mxf-dv</instantiationDigital>
            </xsl:when>
        </xsl:choose>
</xsl:template>
